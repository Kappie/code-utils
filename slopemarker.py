...
def slope_marker(origin, slope, invert=False, size_frac=0.15, pad_frac_x=0.2, pad_frac_y=0.2,
                 text_kwargs={}, poly_kwargs={'facecolor': 'white', 'edgecolor':'black'}, ax=None, custom_rise_label=None, fontsize=10, lw=0.5):
    ...
    if ax.get_xscale() == 'log':
        ...
        x_rise = _text_position(x0+dx, dx_decades*pad_frac_x, scale='log')
    else:
        dx = dx_linear
        x_run = _text_position(x0, dx/2.)
        x_rise = _text_position(x0+dx, pad_frac_x * dx)

    if ax.get_yscale() == 'log':
        log_size = dx_decades * slope
        dy = log_displace(y0, log_size) - y0
        y_run = _text_position(y0, -dx_decades*slope*pad_frac_y, scale='log')
        y_rise = _text_position(y0, log_size/2., scale='log')
    else:
        dy = dx_linear * slope
        y_run = _text_position(y0, -(pad_frac_y * dy))
    ...
    if rise is not None:
        if rise < 0:
            rise = -rise
        ax.text(x_run, y_run, "$%d$" % run, va=va, ha='center', **text_kwargs)
        if custom_rise_label:
            ax.text(x_rise, y_rise, custom_rise_label, ha=ha, va='center', **text_kwargs)
        else:
            ax.text(x_rise, y_rise, "$%d$" % rise, ha=ha, va='center', **text_kwargs)
    else:
        if custom_rise_label:
            ax.text(x_rise, y_rise, custom_rise_label, ha=ha, va='center', **text_kwargs)
        else:
            label = "$%.2f$" % (np.abs(slope))
